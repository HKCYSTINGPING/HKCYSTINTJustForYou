...
function hasSentMessagesSupport(data) {
  return data && data.status === "success" && Array.isArray(data.sent_messages);
}
...
/* ==========================================
   API Calls
   ========================================== */
async function apiFetchMessages(participantId, phoneNumber) {
  ...
  try {
    ...
    console.warn("POST fetch_messages failed:", err);
  }
...
function switchTab(tabName, options = {}) {
  ...
  if (tabName === "sent" && !options.skipFetch) {
    fetchSentMessages();
  }
}

async function fetchSentMessages(button = null) {
  if (!state.participantId || !state.phoneNumber) return;

  const runFetch = async () => {
    const data = await apiFetchMessages(state.participantId, state.phoneNumber);
    if (data.status === "success") {
      applyMessagesFromApi(data);
      renderSentMessages();
      if (!hasSentMessagesSupport(data)) {
        showToast("⚠️ 後端尚未支援送出的留言，請更新 Apps Script 並重新部署", "warning");
      }
    } else {
      showToast(data.message || "無法載入送出的留言", "error");
    }
    return data;
  };

  if (button) {
    await runWithProgress(button, runFetch, null, "📤 載入送出的留言...");
  } else {
    setGlobalLoading(true, 0);
    loadingOverlayText.textContent = "📤 載入送出的留言...";
    try {
      await runFetch();
    } catch (err) {
...
async function handleLogin(e) {
  ...
  try {
    await runWithProgress(
      loginBtn,
      () => apiFetchMessages(participantId, phoneNumber),
      (data) => {
        if (data.status === "success") {
...
async function syncMessagesFromServer(button, loadingText) {
  await runWithProgress(
    button,
    () => apiFetchMessages(state.participantId, state.phoneNumber),
    (data) => {
      if (data.status === "success") {
        applyMessagesFromApi(data);
        renderInbox();
...
async function handleRefreshSent() {
  try {
    await fetchSentMessages(refreshSentBtn);
  } catch (err) {
    showToast("連線失敗，請稍後再試", "error");
    ...
    await runWithProgress(
      ...
      async (data) => {
        if (data.status === "success") {
          ...
          const refreshed = await apiFetchMessages(state.participantId, state.phoneNumber);
          if (refreshed.status === "success") {
            applyMessagesFromApi(refreshed);
          }

          renderSentMessages();
          switchTab("sent", { skipFetch: true });
